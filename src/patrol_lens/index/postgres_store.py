...
class PostgresIndexStore:
            ...
            cursor.execute(
                """INSERT INTO pl_embeddings
                   ...
                   ON CONFLICT(id) DO UPDATE SET
                     ...
                     embedding_hash = EXCLUDED.embedding_hash, embedding = EXCLUDED.embedding,
    ...
    def _ensure_vector_index(cursor: Any, dimensions: int) -> None:
        # The integer is validated above before interpolation; it is a type
        # modifier and cannot be passed as a SQL parameter.
        name = f"pl_embeddings_cosine_{dimensions}"
        cursor.execute(
    ...
    def embedding_records(self, modality: str, model: str) -> list[tuple[str, list[float]]]:
        with self.db.cursor() as cursor:
            cursor.execute(
                """SELECT id, embedding FROM pl_embeddings
                   WHERE modality = %s AND model_version = %s ORDER BY id""",
                (modality, model),
            )
            rows = cursor.fetchall()
    ...
    def search_vectors(
        ...
        *,
        ...
        with self.db.cursor() as cursor:
            cursor.execute(
                f"""SELECT e.*,
                          1 - ((e.embedding::vector({dimensions})) <=> (%s::vector({dimensions}))) AS similarity
                   FROM pl_embeddings e
                   WHERE e.modality = %s AND e.model_version = %s AND e.dimensions = %s
                   ORDER BY (e.embedding::vector({dimensions})) <=> (%s::vector({dimensions}))
                   LIMIT %s""",
